...
if hash php-cs-fixer 2>/dev/null; then
  php-cs-fixer fix "$PATH_ROOT/bin/" --level=psr2 --fixers=unused_use,ordered_use,phpdoc_params,concat_with_spaces,-concat_without_spaces
...
if [ -x "$PATH_PHPCS" ]; then
  $PATH_PHPCS --standard="$PATH_ROOT/data/cs-ruleset.xml" --encoding=utf-8 "$PATH_ROOT/bin/"
